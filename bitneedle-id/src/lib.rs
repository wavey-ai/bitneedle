...
//! ```
//!
//! IDs name objects. They are not content commitments. Exact content identity
//! belongs in explicitly named hashes such as `recordCommitment` or
...
pub enum BitneedleIdError {
    ...
    #[error(
        "ULID body must contain exactly {ULID_TEXT_LENGTH} characters; found {actual}"
    )]
...
fn parse_parts(value: &str) -> Result<(BitneedleIdKind, Ulid), BitneedleIdError> {
    ...
    let ulid =
        Ulid::from_string(body).map_err(|error| BitneedleIdError::InvalidUlid(error.to_string()))?;
