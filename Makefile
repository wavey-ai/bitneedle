...
record-wasm:
	RUSTFLAGS="$(RUSTFLAGS) -C link-arg=--initial-memory=$(RECORD_WASM_INITIAL_MEMORY)" \
		$(WASM_PACK) build record-wasm --target $(WASM_TARGET) --out-dir pkg

record-cut-wasm:
	RUSTFLAGS="$(RUSTFLAGS) -C link-arg=--initial-memory=$(RECORD_AUTHOR_WASM_INITIAL_MEMORY)" \
